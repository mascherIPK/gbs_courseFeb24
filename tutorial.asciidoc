...
[source,sh]
----
ls gbs_reads | grep -c 'bam$'
----

. Count the number of mapped reads with a minimum quality score of 20 for the HOR 337 sample and compare it to the read count of that sample.
+
[source,sh]
----
samtools view -cq 20 
...
. Calculate the mapping rates (i.e. proportion of mapped reads) for all samples.
+
[source,sh]
----
