...
. Connect with https://en.wikipedia.org/wiki/PuTTY[PuTTY] to the server slurm-14.
...
[source,sh]
----
samtools view -cq20 gbs_reads/HOR_337.bam
...
[source,sh]
----
ls $PWD/gbs_reads/*bam | sort > bam_list.txt
----
...
head -n 100 $vcf | less -S
grep -v '^##' $vcf | head -n 100 | column -t | less -S
----
...
. Review the filtered VCF file.
+
[source,sh]
----
grep -v '^##' bcftools_SNP_calling_filtered.vcf | head -n 100 | column -t | less -S
----
...
<4> The https://cran.r-project.org/web/packages/countrycode/index.html[countrycode] package will be used to make country abbreviations to geographic regions.
<5> https://cran.r-project.org/web/packages/openxlsx/index.html[openxlsx] is used to read Excel spreadsheet.
